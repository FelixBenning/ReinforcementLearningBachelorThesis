...
To illustrate the uses of such a framework, I have selected a few examples from \textcite{whiteRealApplicationsMarkov1985}: \fxnote{more diverse examples}
\begin{enumerate}
...
\begin{definition} Let \(\cM=(\cX,\cA,\cP)\) be a MDP. Assume an exploring start, i.e. select \(X_0\) such that \(\Pr({X_0=x})>0\) for all states \(x\). Then every behavior \(\pi\) results in a stochastic process \(((X_t,A_t,R_{t+1}), t\in \N_0)\).
...
\begin{remark}
	...
	\begin{itemize}[noitemsep]
		\item impose regularity conditions which would ensure measuability,
		\item use outer integrals to avoid measuability issues altogether,
		\item or extend notions of measuability.
	\end{itemize}
...
\end{definition}
\begin{remark} The reward in a terminal state is zero by convention, i.e. \(x\) terminal state implies forall actions \(a\in\cA_x\) that \(R_{(x,a)}=0\)
\end{remark}
...
\end{align*}
then \((\tilde{X}_t,\tilde{A}_t,\tilde{R}_{t+1}, t\in\N_0)\) is an \fxfatal*{or realization or "Markov Action Process"? or something else?}{evaluation} of the MDP with the (stationary!) policy \(\pi\).
\end{proof}
