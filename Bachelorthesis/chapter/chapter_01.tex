...
The undiscounted case will be of less interest (c.f. \ref{bootstrapping and discounting}), but one possible way to handle that case, is to assume epsisodic MDPs (c.f. \ref{def: episodic}). In order to define value functions, we will only need the expected return to be well defined for now.
...
\begin{proof} Let \(\pi\in\statPolicy\), then:
	...
	\]
	The rest follows from proposition \ref{expand Q^pi}. 
...
\begin{remark}\label{T^pi unique}
	For all stationary policies \(T^\pi V^\pi=V^\pi \) holds and for deterministic stationary policies \(T^\pi Q^\pi=Q^\pi\) holds (c.f. corollary \ref{V^pi,Q^pi relation}).

	\(T^\pi\) meets the requirements of the Banach fixed point theorem \fxnote{proof BFT? - appendix?} for \({\gamma<1}\). This implies that the fixed points above are \emph{unique} fixed points and can be approximated with the canonical iteration. 
\end{remark}
...
Now that we proved the uniqueness of the optimal value functions, we need to ask the question whether this supremum can be attained with a single policy. And if not, if it can be approximated over all states by the same sequence of policies (c.f. \ref{finite outmatching}). 
...
\begin{proof}
	\begin{align*}
		...
		&=\sum_{t=0}^\infty \gamma^t \sum_{(y,a)\in\cX\times\cA}
		\begin{aligned}[t]
			&\E^{\pi^x}[R_{t+1}\mid X_t=y, A_t=a, X_0=x]\\
			&\qquad\cdot\Pr^{\pi^x}(X_t=y,A_t=a\mid X_0=x)
		\end{aligned}\\
