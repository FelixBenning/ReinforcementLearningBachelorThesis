...
\end{itemize}
This allows us to define the \emph{return}: \fxerror{does this series exist? argument neccessary? c.f. assumption 1 - enough?}
\[
...
\begin{definition} Let \(\cM=(\cX,\cA,\cP)\) be an MDP. Assume an exploring start, i.e. select \(X_0\) such that \(\Pr({X_0=x})>0\) for all states \(x\). Then every behaviour \(\pi\) results in a stochastic process \(((X_t,A_t,R_{t+1}), t\in \N_0)\).
...
\end{align*}
\footnotetext[1]{Well defined because \(\Pr(X_0=x)>0\). Note that the definition is independent of the distribution of \(X_0\) and \(V^\pi\) can thus be defined as an inherent property of the MDP even when the actual start of the MDP is not exploring.}
...
\begin{remark}
	While the following proofs do not require discrete state and action spaces per se (as all the sums could just be replaced by integrals), the optimal value functions need not be measurable \parencite[157]{putermanMarkovDecisionProcesses2005} \fxnote{he quotes Blackwell 1965 - quote blackwell directly?}. To remedy this \citeauthor{putermanMarkovDecisionProcesses2005} suggests to either
	\begin{itemize}[noitemsep]
...
To avoid clutter we will from now on assume an underlying MDP with the accompanying definitions and notation.
...
With this relation we can use the Banach fixed point theorem (BFT) for the first time. But to do that we first need to make an assumption.
...
\begin{proof} \ref{i:1} The smaller or equal part is easy:
	...
	\end{align}
	Assume (\ref{inequality}) is a true inequality for some \(x\in\cX\). Since the suprema in (\ref{last eq}) can be arbitrarily closely approximated we get \fxwarning{exist in set? clutters up the equation making it harder to read, but is math. more correct}
	\[ 
...
\subsection{Bootstrapping and Discounting}
