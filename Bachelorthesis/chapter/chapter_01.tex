...
\begin{proof} We will use \(\E[X\mid A]=\sum_{n\in\N} \E[X\mid A \cap B_n] \Pr(B_n \mid A) \) with \({\biguplus_{n\in\N}B_n=\Omega }\) (c.f. \ref{appx1}) quite a bit in this proof.
...
\begin{align*}
	...
	\cdot \Pr(A_1=b \mid X_0=x,A_0=a, X_1=y) 
	\end{aligned}
	\\
...
\[
	...
	+ \gamma \sum_{y\in\cX} \Pr(X_1=y, A_0=a \mid X_0=x) V^\pi(y)
\]	
...
\begin{proof} Be \(\pi\in\statPolicy\), then:
	\begin{align}
		...
		\pi(a\mid x)\\
		&=\sum_{a\in\cA} \left(r(x,a)
		+\gamma\sum_{y\in\cX}p(y\mid x,a)V^\pi(y)\right)\pi(a\mid x)\nonumber\\
		&=\sum_{a\in\cA}r(x,a)\pi(a\mid x) 
		+ \gamma\sum_{(y,a)\in\cX\times\cA} V^\pi(y) p(y\mid x,a)\pi(a\mid x)\label{stoch pi: fixpoint proof}\\
		&=\E[r(x,A_0)\mid X_0=x] + \gamma\sum_{(y,a)\in\cX\times\cA} V^\pi(y)
...
\begin{definition} 
	...
	\[
		...
		+ \gamma \sum_{y\in\cX} \Pr(X_1=y, A_0=a \mid X_0=x) V(y)
	\]
\end{definition}


\begin{remark}\label{T^pi unique}
	For all deterministic stationary policies holds \(T^\pi V^\pi=V^\pi \) and \(T^\pi Q^\pi=Q^\pi\). And for (stoch.) stationary policies \(\tilde{T}V^\pi=V^\pi\) (c.f. \ref{V^pi,Q^pi relation}).
...
\end{remark}
	\textcolor{red}{Also prove that no non-stationary policy exists which can be optimal without a deterministic policy which can match it?}
	
	From \ref{ii:3} follows a first heuristic to find an optimal value function. This heuristic of course only works, if the maximum exists. This is not a problem in finite MDPs. 
\begin{definition}
...
\end{prop}
\begin{proof}
	Be \(\varepsilon>0 \) arbitrary,  define \(\delta\coloneqq \vep (1-\gamma) \), since:
	\[
	...
	\]
	This defines a mapping \(\pi^\vep\colon \cX\to\cA\) for which:
	\begin{align*}
		T^{\pi^\vep} V^*(x)
	...
	\end{align*}
	By induction we get 
	\((T^{\pi^\vep})^n V^*(x)\ge (V^*-(\delta\sum_{k=0}^n\gamma) \mathbf{1}) (x)\) with the induction step:
	\begin{align*}
		(T^{\pi^\vep})^{n+1} V^*(x)
		&= (T^{\pi^\vep})^n (V^*-\delta)(x)\\
		&\lxeq{\text{affin}} \dots %TODO
	\end{align*}
	This implies
	\begin{align*}
		V^{\pi^\vep}(x)&=\lim_{n\to\infty} (T^{\pi^\vep})^n V^* 
		\ge V^*(x)-\delta\lim_{n\to\infty}\sum_{k=0}^n\gamma\\
		&=V^*(x)-\delta/(1-\gamma)=V^*(x)-\vep
	\end{align*}
\end{proof}
